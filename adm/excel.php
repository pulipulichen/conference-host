...
<table width="200%" border="1" cellpadding="2" cellspacing="0" bordercolor="#999999">
	...
	do { 
			...
			$query_review1 = sprintf("SELECT item1, item2, item3, item4, item5, item6, item7, item8, item9 FROM review WHERE serial ='%s'"
                                , $row_referee['referee1'].$row_paper['paper_serial']);
			$review1 = mysql_query($query_review1, $conn) or die(mysql_error());
			$row_review1 = mysql_fetch_assoc($review1);
			$query_review2 = sprintf("SELECT item1, item2, item3, item4, item5, item6, item7, item8, item9 FROM review WHERE serial ='%s'"
                                , $row_referee['referee2'].$row_paper['paper_serial']);
